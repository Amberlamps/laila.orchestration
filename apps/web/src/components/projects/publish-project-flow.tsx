 ...
 *
 * Manages the publish validation flow for projects. First validates all
...
import { usePublishProject, useValidateProject } from '@/lib/query-hooks';
...
export interface PublishProjectFlowProps {
  projectId: string;
  projectName: string;
  open: boolean;
  onClose: () => void;
...
export function PublishProjectFlow({
  ...
  /**
   * Step 1: Validate the project structure without changing state.
   */
  ...
  }, [projectId, validateProject, onClose]);

  /**
   * Step 2: Publish the project after validation passes and user confirms.
   */
  ...
  const handleSuccessClose = useCallback(() => {
    toast.success('Project Published', `"${projectName}" has been transitioned to Ready status.`);
    onPublished?.();
    onClose();
  }, [projectName, onPublished, onClose]);
  ...
  if (flowState === 'validating') {
    return (
      ...
      >
        <DialogContent size="sm" aria-describedby="publish-validating-desc">
          <DialogHeader>
            <DialogTitle>Publishing Project</DialogTitle>
            <DialogDescription id="publish-validating-desc">
              Validating project structure before publishing.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col items-center gap-4 py-6">
            ...
            <p className="text-sm text-zinc-600">Validating project structure...</p>
          </div>
        </DialogContent>
      </Dialog>
    );
  ...
  if (flowState === 'errors') {
    return (
      ...
      >
        <DialogContent size="md" aria-describedby="publish-errors-desc">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-amber-500" aria-hidden="true" />
              Validation Issues Found
            </DialogTitle>
            <DialogDescription id="publish-errors-desc">
              The following issues must be resolved before &ldquo;{projectName}&rdquo; can be
              published.
            </DialogDescription>
          </DialogHeader>
  ...
  if (flowState === 'publishing') {
    return (
      ...
      >
        <DialogContent size="sm" aria-describedby="publish-publishing-desc">
          <DialogHeader>
            <DialogTitle>Publishing Project</DialogTitle>
            <DialogDescription id="publish-publishing-desc">
              Transitioning project to Ready status.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col items-center gap-4 py-6">
            ...
            <p className="text-sm text-zinc-600">Publishing project...</p>
          </div>
        </DialogContent>
      </Dialog>
    );
  ...
  if (flowState === 'confirming') {
    return (
      ...
      >
        <DialogContent size="sm" aria-describedby="publish-confirm-desc">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <CheckCircle2 className="h-5 w-5 text-green-500" aria-hidden="true" />
              Validation Passed
            </DialogTitle>
            <DialogDescription id="publish-confirm-desc">
              &ldquo;{projectName}&rdquo; has passed all validation checks and is ready to be
              published. This will transition it to Ready status, making it available for work
              assignment.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={runPublish}>Publish Project</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    );
  ...
  return (
    ...
    >
      <DialogContent size="sm" aria-describedby="publish-success-desc">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <CheckCircle2 className="h-5 w-5 text-green-500" aria-hidden="true" />
            Project Published
          </DialogTitle>
          <DialogDescription id="publish-success-desc">
            &ldquo;{projectName}&rdquo; has been validated and transitioned to Ready status. It is
            now available for work assignment.
          </DialogDescription>
        </DialogHeader>
