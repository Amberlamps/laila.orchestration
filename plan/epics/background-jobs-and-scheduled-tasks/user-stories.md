...
| [Implement DAG Reconciler Lambda](./user-stories/implement-dag-reconciler-lambda/tasks.md)                     | Lambda function that performs full-graph consistency checks and fixes status inconsistencies | In Progress (laila-agent-3) | 3 tasks | None         |
...
| [Implement SQS Status Propagation Consumer](./user-stories/implement-sqs-status-propagation-consumer/tasks.md) | SQS-triggered Lambda that processes cascading status re-evaluation events                    | In Progress (laila-agent-2) | 3 tasks | None         |
